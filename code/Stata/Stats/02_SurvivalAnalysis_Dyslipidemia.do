*Survival analysis 
	...
	* Table 2: Unadjusted and adjusted hazards ratios for factors associated with dyslipidemia: low certainty - men & women 

		* AnalyseSurv table 
			use "$clean/analyseSurv", clear	
			
		*Update time-varing variable for dyslipidemia
			listif patient start18 end dl1_y dl1_y_tvc dl1_d if dl1_y_tvc==1, id(patient) sort(patient start18) n(3) seed(3) nolab
			replace dl1_y_tvc = 1 if end == dl1_d & dl1_y ==1
			listif patient start18 end dl1_d dl1_y dl1_y_tvc if dl1_y ==1, id(patient) sort(patient start18) nolab seed(1) n(3) sepby(dl1_y_tvc)
			assert dl1_y_tvc ==1 if end == dl1_d
			bysort patient dl1_y_tvc (start18): gen n = _n
			listif patient start18 end dl1_d dl1_y dl1_y_tvc n if dl1_y ==1, id(patient) sort(patient start18) nolab seed(1) n(3) sepby(patient dl1_y_tvc)
			count if end != dl1_d & n ==1 & dl1_y ==1 & dl1_y_tvc ==1
			replace dl1_y_tvc = 0 if end != dl1_d & n ==1 & dl1_y ==1 & dl1_y_tvc ==1
			assert end == dl1_d if n ==1 & dl1_y_tvc ==1 
			assert dl1_y_tvc ==1 if end == dl1_d
			drop n
		...
		* Model 1: adjusted for PTSD, sociodemographic characteristics and year 
			...
			* Model 3: adjusted for PTSD, sociodemographic characteristics, year and CVD risk factors and HIV 
			stcox i.ptsd1_y_tvc ib3.age ib2.sex i.year i.popgrp i.dm1_y_tvc i.ht1_y_tvc i.hiv1_y_tvc  
			...
			dropcoef(0b.ptsd1_y_tvc 0b.dm1_y_tvc 0b.ht1_y_tvc 0b.hiv1_y_tvc ///
			h.dm1_y_tvc h.ht1_y_tvc h.hiv1_y_tvc ) sort(number0 id0)	
			
			* Model 4: adjusted for PTSD, sociodemographic characteristics, year, psychiatric comorbidity, CVD risk factors and HIV 
			stcox i.ptsd1_y_tvc ib3.age ib2.sex i.year i.popgrp i.othanx1_y_tvc i.org1_y_tvc i.su1_y_tvc i.psy1_y_tvc i.mood1_y_tvc i.sleep1_y_tvc i.omd1_y_tvc i.dm1_y_tvc i.ht1_y_tvc i.hiv1_y_tvc  
			regtable ptsd1_y_tvc age sex year popgrp othanx1_y_tvc org1_y_tvc su1_y_tvc psy1_y_tvc mood1_y_tvc sleep1_y_tvc omd1_y_tvc dm1_y_tvc ht1_y_tvc hiv1_y_tvc, ///
			heading number(0) merge("$temp/hrMACE") varsuffix(4) estlab("aHR (95% CI)") keep(var est label) ///
			dropcoef(0b.ptsd1_y_tvc 0b.othanx1_y_tvc 0b.org1_y_tvc 0b.su1_y_tvc 0b.psy1_y_tvc 0b.mood1_y_tvc 0b.sleep1_y_tvc 0b.omd1_y_tvc 0b.dm1_y_tvc 0b.ht1_y_tvc 0b.hiv1_y_tvc ///
			h.othanx1_y_tvc h.org1_y_tvc h.su1_y_tvc h.psy1_y_tvc h.mood1_y_tvc h.sleep1_y_tvc h.omd1_y_tvc h.dm1_y_tvc h.ht1_y_tvc h.hiv1_y_tvc ) sort(number0 id0)		
		...
		* Export table 
			...
			putdocx text ("Table 4: Unadjusted and adjusted hazard ratios for variables associated with dyslipidemia"), font("Arial", 9, black) bold
			...
			putdocx save "$tables/Table 4_Dyslipidemia.docx", replace	